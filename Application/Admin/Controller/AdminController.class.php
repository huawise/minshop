...
class AdminController extends Controller {
    ...
    protected function _initialize(){
		...
		$peradmin = M('permissions')->where('perid = '.$perid)->find();
		if($peradmin['system']){
			$peradmin['system'] = explode(',',$peradmin['system']);
		}
		if($peradmin['order']){
			$peradmin['order'] = explode(',',$peradmin['order']);
		}
		if($peradmin['product']){
			$peradmin['product'] = explode(',',$peradmin['product']);
		}
		if($peradmin['list']){
			$peradmin['list'] = explode(',',$peradmin['list']);
		}
		
		$jstag = $_SERVER['REQUEST_URI'];
		
		$this->assign('adminid', UID);
		$this->assign('peradmin', $peradmin);
		$this->assign('jstag', $jstag);

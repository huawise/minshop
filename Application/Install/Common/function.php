...
function check_env(){
	...
	//	'mysql'   => array('MYSQL版本', '5.5', '5.5+', mysql_get_server_info(), 'success'), //PHP5.5不支持mysql版本检测
	...
	//PHP环境检测
	if(($items['php'][3] < $items['php'][1]) || ($items['php'][3] > '5.5.9')){
		$items['php'][4] = 'error';
		session('error', true);
	}
...
function register_administrator($db, $prefix, $admin, $auth){
	...
	$sql = "INSERT INTO `[PREFIX]member` VALUES ".
		   "('1', '[NAME]', '0', '0', '', '0', '1', '0', '[TIME]', '0', '[TIME]', '1','1');";
	$sql = str_replace(
